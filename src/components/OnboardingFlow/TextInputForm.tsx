import { ForwardNavButton } from "@/components/ForwardNavButton";
  ...
  import { ComponentProps, useCallback, useEffect, useRef, useState } from
  "react";
  ...
  export function TextInputForm<A extends AnswerValue>({
    ...
    const parsedValue = hasInput ? validationSpec.parse(preprocessedValue) :
  null;
  ...
  export const integerValidation = (options: {
    ...
    validate: (value) => {
      if (value < options.min || value > options.max) {
        return `Please enter a number between ${options.min} and
  ${options.max}.`;
      }
      return null;
    },
  ...
  const TextInput = styled.input`
    ...
    font-size: ${
      /*
        ...
        Safari to zoom into the input field when it's focused, which in turn
  will
